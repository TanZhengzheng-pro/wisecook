  ...
  <div id="app">
    ...
    <nav class="navbar">
      ...
      <div class="navbar-right">
        <a href="#" class="nav-item">Recipe Search</a>
...
<!-- 加载更多按钮 -->
<div v-if="currentPage * pageSize < allRecipes.length" style="text-align: center; margin-top: 20px;">
  <button @click="loadMore" style="padding: 10px 20px; font-size: 16px; border-radius: 10px; background-color: #ffdb63; border: none; cursor: pointer;">
    Load More
  </button>
</div>
  ...
  <!-- Random食谱展示部分 -->
  <div class="random-recipe" v-if="randomRecipe.length > 0">
    <div 
        @click="openRandomRecipe(randomrecipe.strYoutube)">       
       <h3> {{randomRecipe.title}} </h3>
       <p><strong>Meal:</strong> {{ randomrecipe.title }} </p> 
        <!-- <p><strong>Area:</strong> {{ randomrecipe.strArea }} </p> -->
...
<!--           { randomrecipe.strMeasure1 }
          { randomrecipe.strIngredient2 }{ randomrecipe.strMeasure2 }
          { randomrecipe.strIngredient3 }{ randomrecipe.strMeasure3 }
          { randomrecipe.strIngredient4 }{ randomrecipe.strMeasure4 }
          { randomrecipe.strIngredient5 }{ randomrecipe.strMeasure5 }
 -->
      </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async fetchRecipes() {
      ...
      // const apiUrl = "https://api.spoonacular.com/recipes/complexSearch?number=2";
      ...
      try {
        const response = await axios.get(apiUrl, {
          params: {
            ...
            // includeIngredients: ingredientInputs.join(","), // 确保食谱中包含输入的食材
            number: 2,  // 返回100个结果
            apiKey: apiKey,
          },
        });
...
// 提取所有食谱
const randomRecipe = response.data.meals.map((recipe) => ({
  id: recipe.idMeal,
  title: recipe.strMeal,
  image: recipe.strMealThumb,
  ingredient: recipe.strIngredient1, // 示例提取某个原料字段
...
this.randomRecipe = {
  ...
  ingredient: firstRecipe.strIngredient1, // 提取第一个原料字段
